    ...
    <div class="container">
        ...
        <div class="task-zone">
            <div class="drop-zone" @drop="onDrop($event, 'todo')" @dragenter.prevent @dragover.prevent>
                <h1>To-Do</h1>
                <div class="drag-el" draggable @dragstart="onStart($event,task )" v-for="task in taskTodo" :key="task.id" >
                   <!-- {{ task.title }} -->
                   <span v-if="editTask != task.id">{{task.title}}</span>
                   <input v-else class="editTask= task.id"  type="text" v-model="task.title">
